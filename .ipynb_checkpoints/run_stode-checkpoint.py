...
def train(loader, model, optimizer, criterion, std, mean, device):
    ...
    for idx, (inputs, targets) in enumerate(tqdm(loader)):
        ...
        # loss = logcosh(targets, outputs)
        loss.backward()
        optimizer.step()
        # print('outputs, targets,loss',outputs[0,:5,0], targets[0,:5,0],loss)
        ...
        # print(mae_loss,rmse_loss,mape_loss)
...
def main(args):
    ...
    # scheduler = StepLR(optimizer, step_size=50, gamma=0.5)  #attention!!!!!!!!!!!!
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss, train_rmse, train_mae, train_mape = train(train_loader, net, optimizer, criterion, train_std, train_mean,
                                                        device)
        print('Evaluating...')
        # train_rmse, train_mae, train_mape = eval(train_loader, net, train_std, train_mean, device)
        ...
        if valid_mape <= val_mape_min:
            ...
            #     log_string(
            #         log,
            #         f'val loss not exceed {val_loss_min:.4f}, now {mvalid_loss:.4f}, '
            #         f'save model to {args.save + "exp_" + str(args.expid) + "_" + str(round(mvalid_loss, 2)) + "_unbest_model.pth"}'
            #     )
            #     cur_model_wts = engine.model.state_dict()
            #     torch.save(cur_model_wts,
            #                args.save + "epoch_" + str(i) + "_" + str(round(mvalid_loss, 2)) + "_unbest_model.pth")
            wait += 1
            if wait==30:
